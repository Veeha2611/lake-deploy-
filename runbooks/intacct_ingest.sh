...
# -------------------------
LOCK_DIR="${HOME}/.intacct_ingest.lock"
LOCK_INFO="${LOCK_DIR}/info.txt"
...
acquire_lock() {
  if mkdir "${LOCK_DIR}" 2>/dev/null; then
    ...
    echo "ERROR: ingest already running (lock exists at ${LOCK_DIR})." >&2
    if [ -f "${LOCK_INFO}" ]; then
      echo "Lock info:" >&2
      cat "${LOCK_INFO}" >&2 || true
    fi
    exit 2
  fi
}
...
days = int(os.environ.get("GL_ENTRIES_LOOKBACK_DAYS", "365"))
...
append_summary() {
  # Usage: append_summary "label" "value"
  local k="$1"
  local v="$2"
  printf "%s: %s\n" "$k" "$v" >> "$SUMMARY_FILE"
}
...
# initialize summary
: > "$SUMMARY_FILE"
append_summary "run_date" "$RUN_DATE"
append_summary "run_timestamp" "$RUN_TIMESTAMP"
append_summary "company" "$COMPANY_ID"
append_summary "endpoint" "$INTACCT_ENDPOINT"
append_summary "glentry_filter" "$GL_ENTRIES_QUERY"
...
notify_failure() {
  ...
  aws s3 cp "$BASE_DIR/failure.txt" \
    "${S3_BUCKET}/${S3_HEARTBEAT_PREFIX}/failure_${RUN_TIMESTAMP}.txt" || true
}
...
############################################################
refresh_catalog() {
  local query="ALTER TABLE raw_intacct.intel_gl_entries ADD IF NOT EXISTS PARTITION (run_date='${RUN_DATE}') LOCATION 's3://gwi-raw-us-east-2-pc/raw/intacct_json/gl_entries/run_date=${RUN_DATE}/'"
  aws athena start-query-execution \
    --query-string "$query" \
    --result-configuration "OutputLocation=s3://gwi-raw-us-east-2-pc/athena-results/" \
    --work-group primary \
    --region us-east-2 >/dev/null

  aws glue start-crawler --name intacct_gl_entries_json_crawler >/dev/null
}

refresh_catalog

############################################################
# 10. SUCCESS HEARTBEAT
############################################################
...
############################################################
# 11. FINAL SUMMARY (always runs at end if reached)
############################################################

append_summary "status" "success"
append_summary "finished_utc" "$(date -u +%Y-%m-%dT%H:%M:%SZ)"

echo "SUMMARY: run=${RUN_DATE} ts=${RUN_TIMESTAMP} status=success glentry_filter=${GL_ENTRIES_QUERY}"

# upload summary to S3 next to heartbeat prefix (non-fatal)
aws s3 cp "$SUMMARY_FILE" "${S3_BUCKET}/${S3_HEARTBEAT_PREFIX}/summary_${RUN_TIMESTAMP}.txt" >/dev/null 2>&1 || true
