...
case "$ENV_NAME" in
  dev)
    ...
    COMPANY_ID="$COMPANY_ID_DEV"; USER_ID="$WS_USER_ID_DEV"; USER_PASSWORD="$WS_USER_PASSWORD_DEV"
    ;;
  sandbox)
    : "${COMPANY_ID_SANDBOX:?missing}"; : "${WS_USER_ID_SANDBOX:?missing}"; : "${WS_USER_PASSWORD_SANDBOX:?missing}"
    COMPANY_ID="$COMPANY_ID_SANDBOX"; USER_ID="$WS_USER_ID_SANDBOX"; USER_PASSWORD="$WS_USER_PASSWORD_SANDBOX"
    ;;
  prod)
    : "${COMPANY_ID_PROD:?missing}"; : "${WS_USER_ID_PROD:?missing}"; : "${WS_USER_PASSWORD_PROD:?missing}"
    COMPANY_ID="$COMPANY_ID_PROD"; USER_ID="$WS_USER_ID_PROD"; USER_PASSWORD="$WS_USER_PASSWORD_PROD"
    ;;
  *)
    echo "usage: $0 {dev|sandbox|prod}" >&2
    exit 2
