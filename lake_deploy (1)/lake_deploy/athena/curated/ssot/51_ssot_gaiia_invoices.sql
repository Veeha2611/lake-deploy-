-- SSOT scaffold for Gaiia invoices (raw JSON payloads)
-- NOTE: record_json is empty when auth fails; exceptions capture those rows.
CREATE OR REPLACE VIEW curated_core.gaiia_invoices_curated_raw AS
SELECT
  record_id,
  record_json,
  error_json,
...
FROM curated_core.gaiia_invoices;

CREATE OR REPLACE VIEW curated_recon.gaiia_invoices_exceptions AS
SELECT
  CASE
