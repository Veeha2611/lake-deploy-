-- Platt billing MRR at customer-month grain (latest month only, parent-billed).
-- Source: raw_platt.idetail_raw + raw_platt.iheader_raw (latest dt only).
-- Purpose: authoritative per-customer billing snapshot for SSOT + AI.
CREATE OR REPLACE VIEW curated_core.v_platt_billing_customer_month_latest AS
...
SELECT
  b.period_month,
  b.customer_id,
  SUM(b.line_total) AS mrr_total_customer_month
FROM base b
WHERE b.period_month = (SELECT period_month FROM latest_month)
GROUP BY b.period_month, b.customer_id;
