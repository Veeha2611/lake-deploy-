...
LIMIT 200000;
    ...
    console.log('🔍 RevenueReport SQL:', sql);
    ...
    // Parse Query Layer response
    const aiResponse = queryResponse;
    
    // Build CSV
    const columns = ['customer_id', 'customer_name', 'has_active_service', 'is_test_internal', 'system_id', ...months];
    ...
    const filename = `${export_name || 'revenue_report'}_${timestamp}.csv`;
    ...
    await base44.asServiceRole.entities.ReportRun.create({
      report_name: export_name || 'RevenueReport',
      report_mode: 'RevenueReport',
    ...
    console.error('❌ runRevenueReport error:', error);
    return Response.json({ 
      error: error.message,
      stack: error.stack 
    }, { status: 500 });
