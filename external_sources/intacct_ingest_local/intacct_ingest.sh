...
SENDER_REDACTED INTACCT_SENDER_PASSWORD}"

WS_USER_ID="${INTACCT_WS_USER_ID:?Missing INTACCT_WS_USER_ID}"
WS_USER_REDACTED INTACCT_WS_USER_PASSWORD}"
...
S3_GL_ENTRIES_RUN_DATE_PREFIX="${S3_BUCKET}/${S3_JSON_PREFIX}/gl_entries/run_date=${RUN_DATE}"
...
EOF
  ...
  aws s3 cp /tmp/gl_entries_metadata.json "${S3_GL_ENTRIES_RUN_DATE_PREFIX}/metadata.json" --region "${AWS_REGION}"
}
...
PY
  ...
  local GL_ENTRIES_JSON_S3_URI="${S3_GL_ENTRIES_RUN_DATE_PREFIX}/gl_entries.json"
...
pull_object_paginated "VENDOR" "vendors"
pull_object_paginated "CUSTOMER" "customers"
pull_gl_accounts
